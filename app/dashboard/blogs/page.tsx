...
export default function ManageBlogsPage() {
  ...
  const { data, mutate, isLoading } = useSWR('/api/blogs?limit=50', fetcher);
    ...
    <main className="min-h-screen bg-gradient-to-br from-background via-background to-secondary/10">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <div className="flex items-center justify-between mb-8">
          <h1 className="text-4xl font-bold text-foreground">Your Blogs</h1>
          <Link
        ...
        {!isLoading && blogs.length > 0 ? (
          <div className="bg-card rounded-xl border border-border overflow-hidden shadow-lg">
            <div className="overflow-x-auto">
              <table className="w-full">
                ...
                <tbody>
                  {blogs.map((blog: any) => (
                    <tr key={blog._id} className="border-b border-border hover:bg-background/30 transition-colors">
                      ...
                      <td className="px-6 py-4 text-right">
                        <div className="flex items-center justify-end gap-2">
                          ...
                          <Link
                            href={`/dashboard/edit-blog/${blog.slug}`}
                            className="p-2 rounded-lg hover:bg-primary/10 text-primary transition-colors"
                            title="Edit"
                          >
                            <Edit size={18} />
          ...
          <div className="text-center py-12 bg-card rounded-xl border border-border">
            <p className="text-muted-foreground mb-6">No blogs published yet.</p>
            <Link
              href="/dashboard/create-blog"
            ...
            >
              <Plus size={20} /> Create Your First Blog
            </Link>
          </div>
        ) : null}
      </div>
